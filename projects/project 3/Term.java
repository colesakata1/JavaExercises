...
public class Term implements Comparable<Term> {
    // weight of the term (number)
    private long weight;
    // Actual text string of the term
    ...
    private static class ReverseWeightOrder implements Comparator<Term> {
        ...
        public int compare(Term v, Term w) {
            return Integer.compare((int) w.weight, (int) v.weight);
        }
    }

        // Prefix-order comparator.
    private static class PrefixOrder implements Comparator<Term> {
        // r is the length of the prefix in characters
        private int r;
        ...
        public int compare(Term v, Term w) {
            ...
            String subw;
            if (v.query.length() >= r) {
                subv = v.query.substring(0, r);
            } else {
                subv = v.query;
            }
            if (w.query.length() >= r) {
                subw = w.query.substring(0, r);
            } else {
                subw = w.query;
            }
